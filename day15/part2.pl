...
my $table = multiplyInput($tbl, 5);

printGrid($table, {});
print "After print grid\n";
...
sub multiplyInput() {
	...
	my $tblRowCount = scalar @$tbl;

	for my $y (0..$by-1) {
		if ($y > 0) {
			push @lgTbl, [] for (1..$tblRowCount);
		}
		for my $x (0..$by-1) {
			unless ($y == 0 && $x == 0) {
				my $appToY = 0;
				for (@$tbl) {
					for (@$_) {
						my $riskLvl = ($_+($x+$y));
						$riskLvl = $riskLvl - 9 if ($riskLvl > 9);
						push @{@lgTbl[$appToY+($y*$tblRowCount)]}, $riskLvl;
					}
					$appToY++;
				}
			}
	...
	print "Return a table with ". scalar(@lgTbl) ." rows, that are ". scalar(@{$lgTbl[0]}) ." wide\n";
	return \@lgTbl;
}
